...
public class MMU {
    ...
    private String toRealLinearAddr(String logicAddr) {
        // TODO
        StringBuilder basis = new StringBuilder(logicAddr.substring(0, 16));
        StringBuilder offset = new StringBuilder(logicAddr.substring(16));
        basis.insert(0, "000000000000").append("0000");
        offset.replace(0, 16, "0000000000000000");
        DataType physicalAddr = alu.add(new DataType(basis.toString()), new DataType(offset.toString()));
        StringBuilder phyAddr = new StringBuilder(physicalAddr.toString());
        phyAddr.replace(0, 12, "000000000000");
        return phyAddr.toString();
    }
    ...
    private String toSegLinearAddr(String logicAddr) {
        // TODO
        int segIndex = getSegIndex(logicAddr);
        return alu.add(new DataType(String.valueOf(memory.getBaseOfSegDes(segIndex))), new DataType(logicAddr.substring(16))).toString();
    }
    ...
    private String toPagePhysicalAddr(String linearAddr) {
        ...
        char[] pageFrame;
        if (TLB.isAvailable) {
            pageFrame = tlb.getFrameOfPage(Integer.parseInt(transformer.binaryToInt(linearAddr.substring(0, 20))));
        } else {
            pageFrame = memory.getFrameOfPage(Integer.parseInt(transformer.binaryToInt(linearAddr.substring(0, 20))));
        }
        return String.valueOf(pageFrame) + linearAddr.substring(20,32);
    }
