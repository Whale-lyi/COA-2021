...
public class Cache {
    ...
    private int map(int blockNO) {
        ...
        int rowNO = Integer.parseInt(transformer.binaryToInt("0" + pAddr.substring(22-log2(SETS))));
        StringBuilder realTag = new StringBuilder(pAddr.substring(0, 22-log2(SETS)));
        while (realTag.length() < 22) realTag.append("0");
        char[] tag = realTag.toString().toCharArray();

        char[] tagInCache = cache.get(rowNO).tag;
        for (int i=0; i<22; i++) {
            if (tag[i] != tagInCache[i]) {
                result = -1;
                break;
            }
        }
        if (result != -1) {
            if (cache.get(rowNO).validBit) result = rowNO;
            else result = -1;
        }
        return result;
    }
